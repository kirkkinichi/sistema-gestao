<x-layout>
    <div class="flex justify-center items-center min-h-screen bg-gray-100">
        <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-4xl">
            <h1 class="text-3xl font-semibold text-center text-gray-800 mb-6">Adicionar Unidade</h1>
            ...
            @else
                <form method="POST" action="/unidades" class="space-y-4">
                    @csrf

                    <div>
                        <label for="nome_fantasia" class="block text-sm font-medium text-gray-700">Nome:</label>
                        <input type="text" name="nome_fantasia" id="nome_fantasia" value="{{ old('nome_fantasia') }}" required class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ffb800] focus:border-[#ffb800]" />
                        @error('nome_fantasia')
                    ...
                    <div>
                        ...
                        <input type="text" name="razao_social" id="razao_social" value="{{ old('razao_social') }}" required class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ffb800] focus:border-[#ffb800]" />
                        @error('razao_social')
                            <small style="color: red; display: block;" class="mt-1">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="cnpj" id="cnpj" value="{{ old('cnpj') }}" required class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ffb800] focus:border-[#ffb800]" />
                        @error('cnpj')
                            <small style="color: red; display: block;" class="mt-1">{{ $message }}</small>
                        @enderror
                    </div>
